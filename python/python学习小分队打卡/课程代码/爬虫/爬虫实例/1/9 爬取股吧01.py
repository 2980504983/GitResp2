    ...
    def parse(self):
        for i in range(1, 13):
            ...
            ul_pattern = re.compile(
                r'<ul id="itemSearchList" class="itemSearchList">(.*?)</ul>',
                re.S)
            ...
            for li in li_list:
                ...
                author_pattern = re.compile(
                    r'target="_blank"><font>(.*?)</font></a><input type="hidden"',
                    re.S)
                author = author_pattern.search(li).group(1)
                # print(author)

                date_pattern = re.compile(r'<cite class="last">(.*?)</cite>',
                                          re.S)
                ...
                if detail_url:
                    detail_url = 'http://guba.eastmoney.com' + detail_url.group(
                        1)
                else:
                    detail_url = ''
