...
class Maoyan:
    ...
    def parse(self):
        ...
        for i in range(10):
            ...
            dl_pattern = re.compile(r'<dl class="board-wrapper">(.*?)</dl>',
                                    re.S)
            ...
            for dd in dd_list:
                ...
                # print(actor)
                play_time_pattern = re.compile(
                    r'<p class="releasetime">(.*?)：(.*?)</p>', re.S)
                play_time = play_time_pattern.search(dd).group(2).strip()
                # print(play_time)

                # 评分
                score_pattern_1 = re.compile(r'<i class="integer">(.*?)</i>',
                                             re.S)
                score_pattern_2 = re.compile(r'<i class="fraction">(.*?)</i>',
                                             re.S)
                score = score_pattern_1.search(dd).group(
                    1).strip() + score_pattern_2.search(dd).group(1).strip()
                # print(score)
